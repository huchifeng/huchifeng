...
void test_align(){
	// 0,4,8,12
	for(int i=0; i<10; i++)
		cerr << i << ":" << QImage(i, 100, QImage::Format_Indexed8).bytesPerLine() << endl;
	//0 4 8 12 16 20
	for(int i=0; i<10; i++)
		cerr << i << ":" << QImage(i, 100, QImage::Format_RGB16).bytesPerLine() << endl;
	// 0 4 8 12 16 20
	for(int i=0; i<10; i++)
		cerr << i << ":" << QImage(i, 100, QImage::Format_RGB32).bytesPerLine() << endl;
}
...
void main(){
	...
	copy(tmp.bits(), img.width(), tmp.bytesPerLine(), img.height(), img.bits(), img.bytesPerLine());
	laplace(tmp.bits()+img.width(), 1, img.width(), tmp.bytesPerLine(), img.height(), img.bits(), 1, img.bytesPerLine());
	tmp.save("tmp.png");
